...
const StyledInputWrapper = styled.div<IStyledWrapperProps>`
  border: 1px solid ${({ theme, error }) => error ? theme.colors.error : theme.colors.light};
  border-radius: 5px;
...
const DatePicker = React.forwardRef(({ value, onChange, name, label, hideLabel = false, error = false }: IDatePickerProps, ref) => {
  const [createdAt, setCreatedAt] = useState<moment.Moment | null>(value ? moment(value) : null);
...
<SingleDatePicker
  ...
  date={createdAt}
  onDateChange={(selectedDate) => setCreatedAt(selectedDate)}
  focused={dateFocus}
  onFocusChange={({ focused }) => setDateFocus(focused)}
/>
*/