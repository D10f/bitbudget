...
import { selectCurrentWallet } from './wallets';
...
export const selectSortedExpenses = createSelector(
  [selectFilteredExpenses, selectFilters],
  (expenses, { sortBy, sortDesc }) => {
    return expenses.sort((a, b) => {
      switch (sortBy) {
        // Apply lowercase if sorting is done by a string value
        case 'title':
        case 'category':
        case 'description':
          return sortDesc
...
export const selectAmountByCategory = createSelector(
  ...
  (expenses) => {
    ...
    return expenses.reduce((acc, expense) => {
      const { category, amount } = expense.data;
